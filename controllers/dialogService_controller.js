...
module.exports = class DialogService_controller {
  async get_stockNews(req, res, next) {
    ...
    try {
      ...
      let stockNews = await result.json();
      stockNews = stockNews.slice(0, 10);
      stockNews = stockNews.map(
        ({
          hasNextPage,
          commentary,
          authortitle,
          ...
          let rebuild_imgUrl = "",
            stockNewLink = stockNew["link"],
            stockNewContent = stockNew["content"];
          if (stockNew.thumbnail !== "") {
            let rebuild_imgUrl_array = stockNew.thumbnail.split("/");
            rebuild_imgUrl = `https://hk.on.cc/hk/bkn/cnt/finance/${rebuild_imgUrl_array[3]}/photo//${rebuild_imgUrl_array[4]}`;
          }
          if (stockNew["link"] !== "") {
            stockNewLink =
              "https://money18.on.cc/finnews/content/related_news/" +
              stockNew["link"].split("/")[4];
          }
          ...
          if (stockNew["content"] !== "") {
            ...
            for (let j = 0; j < replaceWord.length; j++) {
              stockNewContent = stockNewContent.replace(replaceWord[j], "");
            }
          }
          return {
            ...stockNew,
            photo: rebuild_imgUrl,
            link: stockNewLink,
            content: stockNewContent,
          };
        }
      );

      return res.status(200).json(stockNews);
    } catch (error) {
      console.log(error);
  ...
  async get_stockDailyPrice(req, res, next) {
    let { todayHour, todayMinute, todaySecond } = Common.getDayTime();
    let STOCK = req.params.stock;
    //let promises = [];
    let settings = {
    ...
    try {
      const result = await fetch(url, settings);
      const stockDailyPrice = await result.json();
      return res.status(200).json(stockDailyPrice);
    } catch (error) {
      console.log(error);
      res.status(500).json({ error: error.message });
    }
